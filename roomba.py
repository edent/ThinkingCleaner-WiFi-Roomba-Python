...
IP_address = "192.168.0.31" # Change this!
...
# Save the json to
json_file = open("full_status.json", "w")
...
if state != state_old :
	...
	tweet = "Status - "   + status_dict[str(state)] + \
			"\n" + \
			"I've moved " + locale.format_string("%d", distance_new, grouping=True) + " metres (" + locale.format_string("%d", distance, grouping=True) + " metres in total)" + \
			"\n" + \
			"Battery "    + locale.format_string("%d", battery, grouping=False) + "%"
